...
class PerfWidget(tkinter.Frame):
    def __init__(self, frame, decryptFile, perfName, perfValue, defaultValue, rootFrameAppearance):
        ...
        perfFrame = ttkCustomWidget.CustomTtkFrame(self)
        perfFrame.pack(expand=True, fill=tkinter.BOTH)

        self.perfNameLb = ttkCustomWidget.CustomTtkLabel(perfFrame, text=perfName, font=textSetting.textList["font6"], anchor=tkinter.CENTER, relief="solid")
        self.perfNameLb.grid(row=0, column=0, sticky=tkinter.NSEW)
        self.perfValue = perfValue
        self.varPerf = tkinter.DoubleVar()
        self.varPerf.set(self.perfValue)
        self.perfLb = ttkCustomWidget.CustomTtkLabel(perfFrame, textvariable=self.varPerf, font=textSetting.textList["font6"], anchor=tkinter.CENTER, relief="solid")
        self.perfLb.grid(row=0, column=1, sticky=tkinter.NSEW)
        self.perfBtn = ttkCustomWidget.CustomTtkButton(perfFrame, text=textSetting.textList["orgInfoEditor"]["modifyBtnLabel"], style="custom.update.TButton", command=partial(self.editVar, defaultValue), state="disabled")
        self.perfBtn.grid(row=0, column=2, sticky=tkinter.NSEW, padx=(0, 10))
        self.setLabelColor(self.perfNameLb, self.perfLb, perfValue, defaultValue)

        perfFrame.grid_columnconfigure(0, weight=4, uniform="perf")
        perfFrame.grid_columnconfigure(1, weight=1, uniform="perf")
    ...
    def editVar(self, defaultValue):
        ...
        if result.inputFlag:
            ...
            self.varPerf.set(self.perfValue)
            self.setLabelColor(self.perfNameLb, self.perfLb, self.perfValue, defaultValue)
