...
class MdlDecrypt:
    ...
    def updateSmfDetail(self, smfNum, smfDetailNum, valueList, mode):
        try:
            newByteArr = bytearray()
            smfCntIndex = self.allInfoList[smfNum]["smfDetailListIndex"]
            smfDetailCnt = self.byteArr[smfCntIndex]
            index = 0
            ...
            if mode == "insert":
                ...
                smfDetailCnt -= 1
            newByteArr[smfCntIndex] = smfDetailCnt
            ...
            if mode == "modify" or mode == "delete":
                if smfDetailNum + 1 >= len(self.allInfoList[smfNum]["smfDetailList"]):
                    index = self.allInfoList[smfNum]["binInfoIndex"]
                else:
                    index = self.allInfoList[smfNum]["smfDetailList"][smfDetailNum + 1]["smfDetailIndex"]
