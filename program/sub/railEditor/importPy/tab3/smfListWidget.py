...
class SmfListWidget:
    ...
    def setSmfTableData(self):
        ...
        for index, smfInfo in enumerate(self.smfList):
            modelName = smfInfo[0]
...
class EditSmfListWidget(CustomSimpleDialog):
    def __init__(self, master, title, decryptFile, mode, num, headerNameList, smfInfo, rootFrameAppearance):
        self.decryptFile = decryptFile
        self.mode = mode
        self.num = num
    ...
    def body(self, master):
        ...
        if self.decryptFile.game in ["CS", "RS"]:
            ...
            for i, headerName in enumerate(self.headerNameList):
                smfInfoLb = ttkCustomWidget.CustomTtkLabel(master, text=headerName, font=textSetting.textList["font2"])
                smfInfoLb.grid(row=i + 1, column=0, sticky=tkinter.W + tkinter.E)
                if i == 0:
                    ...
                    smfInfoEt.grid(row=i + 1, column=1, sticky=tkinter.W + tkinter.E)

                    if self.mode == "modify":
                        self.varList[i].set(self.smfInfo[i])
                elif i in [1, 2]:
                    ...
                    mb.grid(row=i + 1, column=1, sticky=tkinter.W + tkinter.E)
                else:
    ...
    def setListboxInfo(self, tempList):
        ...
        if len(tempList) > 0:
            for i in range(len(tempList)):
                tempInfo = tempList[i]
                displayTempList.append("{0:02d}→{1}".format(i, tempInfo))
        else:
            displayTempList = [textSetting.textList["railEditor"]["noList"]]
        return displayTempList
...
class EditListElementWidget(CustomSimpleDialog):
    ...
    def body(self, master):
        ...
        for i in range(len(tempInfoLb)):
            ...
            tempLb.grid(row=i + 1, column=0, sticky=tkinter.W + tkinter.E)
            ...
            tempEt.grid(row=i + 1, column=1, sticky=tkinter.W + tkinter.E)

        if self.mode == "insert":
            self.setInsertWidget(master, len(tempInfoLb) + 1)
        super().body(master)
